...
export type ModuleType = 'preferences' | 'presets' | 'theme' | 'optics' | 'artifacts' | 'exposure' | 'lighting' | 'processing' | 'capture' | 'tone' | 'color' | 'texture' | 'details' | 'debug' | 'none';
export type ParameterType = 'presets' | 'language' | 'vibration' | 'ui_overlay' | 'temperature_test' | 'developer_options' | 'camera_selection' | 'focus' | 'chromatic_aberration' | 'bloom' | 'vignette' | 'iso' | 'shutter_speed' | 'ev' | 'torch' | 'torch_strength' | 'aspect_ratio' | 'resolution_setting' | 'fps_setting' | 'temperature' | 'tint' | 'saturation' | 'contrast' | 'blackLevel' | 'highlights' | 'pivot' | 'grain' | 'noise_reduction' | 'sharpening' | 'pixelation' | 'zoom' | 'chroma_shift' | 'tape_jitter' | 'scanlines' | 'hue' | 'lens_distortion' | 'none';
