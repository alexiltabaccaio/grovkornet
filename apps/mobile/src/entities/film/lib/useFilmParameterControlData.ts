    ...
    useShallow((s) => {
      switch (parameter) {
        ...
        // or packages/shared/camera-errors.yaml, then run `npm run codegen`.
                ...
                case 'chroma_bleed':
                  return {
                    chromaBleed: s.chromaBleed,
                    setChromaBleed: s.setChromaBleed
                  };
  ...
  return useMemo((): ParameterControlData => {
    switch (parameter) {
      ...
      // or packages/shared/camera-errors.yaml, then run `npm run codegen`.
            ...
            case 'chroma_bleed':
              return {
                value: film.chromaBleed,
                minValue: 0.0,
                maxValue: 1.0,
                onChange: film.setChromaBleed,
                onUpdateWorklet: filmWorklets.updateChromaBleed,
                hideValueInAuto: false,
                autoValueText: 'AUTO',
                onReset: () => film.setChromaBleed(DEFAULT_CHROMA_BLEED),
                valueFormatter: (v: number) => {
                  'worklet';
                  return `${Math.round((v * 100))}`;
                }
